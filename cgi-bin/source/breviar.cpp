	...
	if((_global_den.denvt == DEN_NEDELA) ||
		...
		if((_global_den.smer > _global_svaty1.smer) ||
			...
			if(_global_den.smer > _global_svaty1.smer){
				 ...
				 *             teraz vlastne obe vetvy vyzeraj˙ rovnako, asi to zjednotÌm Ëasom... (TODO_CLEANUP)
				 */
				...
				/* 2010-05-21: odtiaæto presunutÈ potenci·lne vypisovanie (export) vöednÈho dÚa pred prvÈho sv‰tca, ak je æubovoæn· spomienka */
			}
			else{
	...
	if(m == VSETKY_MESIACE){
		...
		if(d == VSETKY_DNI){
			...
			if(!kontrola(d, m, r)){
				...
				/* 2010-05-21: doplnenÈ pre maÔarËinu */
				else if(_global_jazyk == JAZYK_HU){
					sprintf(pom, "%s %d., %d", nazov_Mesiaca(m - 1), d, r);
				}
				else{
