 ...
 * vypisovalo: `sys_errlist' is deprecated; use `strerror' or `strerror_r' instead
 * a tak som to zmenil na funkciu, bolo treba pridat ten include 
 */
#include <errno.h>
...
http_head(NOPARAMS)
{
	...
	puts("<center><h2>Chyba poèas vyhodnocovania</h2></center>");
}
...
{
	printf("<hr WIDTH=\"100%\">\n");
	printf("<center><font size=-1>© 1999 Juraj Vidéky");
	...
	char	*str;
{
	...
	puts("<p>Poèas vyhodnocovania vašej žiadosti sa vyskytla nasleduj&uacute;ca chyba:");
	puts("<blockquote>");
	printf("%s: %s\n", str, strerror(errno));
	puts("</blockquote>");
